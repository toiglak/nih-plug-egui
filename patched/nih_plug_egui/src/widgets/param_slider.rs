...
use egui_baseview::egui::{
	self, emath, emath::GuiRounding, vec2, Key, Response, Sense, Stroke, TextEdit, TextStyle, Ui, Vec2, Widget,
	WidgetText,
};
	...
	fn slider_ui(&self, ui: &Ui, response: &mut Response) {
		...
		if let Some(click_pos) = response.interact_pointer_pos() {
			...
			//     self.begin_keyboard_entry(ui);
				...
				let proportion = emath::remap_clamp(click_pos.x, response.rect.x_range(), 0.0..=1.0) as f64;
	...
	fn value_ui(&self, ui: &mut Ui) {
		...
		if self.keyboard_entry_active(ui) {
			...
			if ui.is_rect_visible(response.rect) {
				...
				ui.painter()
					.add(egui::epaint::TextShape::new(text_pos, text, visuals.fg_stroke.color));
			}
		}
	}
}
...
impl<P: Param> Widget for ParamSlider<'_, P> {
	fn ui(mut self, ui: &mut Ui) -> Response {
		let slider_width = self.slider_width.unwrap_or_else(|| ui.spacing().slider_width);
		...
		ui.horizontal(|ui| {
			...
			let mut response = ui
				.vertical(|ui| {
					...
					let response = ui.allocate_response(vec2(slider_width, slider_height), Sense::click_and_drag());
					let (kb_edit_id, _) = ui.allocate_space(vec2(slider_width, (height - slider_height) / 2.0));

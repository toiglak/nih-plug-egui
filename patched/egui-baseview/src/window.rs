...
impl<'a> Queue<'a> {
	pub(crate) fn new(bg_color: &'a mut Rgba, close_requested: &'a mut bool, physical_size: &'a mut PhySize) -> Self {
		Self {
			bg_color,
			//renderer,
			//repaint_requested,
	...
	U: 'static + Send,
{
		...
		B: 'static + Send,
	{
		...
		let screen_rect = Rect::from_min_size(
			Pos2::new(0f32, 0f32),
			vec2(open_settings.logical_width as f32, open_settings.logical_height as f32),
		);
	...
	fn apply_window_info(&mut self, window_info: &baseview::WindowInfo) {
		...
		let viewport_info = self.egui_input.viewports.get_mut(&self.viewport_id).unwrap();
	...
	U: 'static + Send,
{
	fn on_frame(&mut self, window: &mut Window) {
		...
		let mut queue = Queue::new(&mut self.bg_color, &mut self.close_requested, &mut self.physical_size);
		...
		for command in viewport_output.commands.iter() {
			match command {
				...
				ViewportCommand::InnerSize(size) => {
					...
					let window_info = baseview::WindowInfo::from_logical_size(size, self.pixels_per_point as f64);
					self.apply_window_info(&window_info);
				}
				_ => {}
			}
		}
		...
		#[cfg(feature = "windows_keyboard_workaround")]
		{
			...
			{
				if !full_output.platform_output.events.is_empty() || full_output.platform_output.ime.is_some() {
					window.focus();
				}
			}
		}
		...
		match &event {
			baseview::Event::Mouse(event) => match event {
				baseview::MouseEvent::CursorMoved {
					...
					self
						.egui_ctx
						.data_mut(|data| data.insert_temp(crate::screen_cursor_position_id(), screen_pos));
				...
				baseview::MouseEvent::WheelScrolled {
					...
					let (unit, mut delta) = match scroll_delta {
						baseview::ScrollDelta::Lines { x, y } => (egui::MouseWheelUnit::Line, egui::vec2(*x, *y)),
					...
					if is_cut_command(self.egui_input.modifiers, event.code) {
						...
						if let Some(clipboard_ctx) = &mut self.clipboard_ctx {
							match clipboard_ctx.get_contents() {
								Ok(contents) => self.egui_input.events.push(egui::Event::Text(contents)),
								Err(err) => {
			...
			baseview::Event::Window(event) => match event {
				...
				baseview::WindowEvent::Focused => {
					self.egui_input.events.push(egui::Event::WindowFocused(true));
					self.egui_input.viewports.get_mut(&self.viewport_id).unwrap().focused = Some(true);
				}
				baseview::WindowEvent::Unfocused => {
					self.egui_input.events.push(egui::Event::WindowFocused(false));
					self.egui_input.viewports.get_mut(&self.viewport_id).unwrap().focused = Some(false);
				}

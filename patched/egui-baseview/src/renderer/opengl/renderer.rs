...
impl Renderer {
	pub fn new(window: &Window, config: GraphicsConfig) -> Result<Self, OpenGlError> {
		...
		let glow_context =
			Arc::new(unsafe { egui_glow::glow::Context::from_loader_function(|s| context.get_proc_address(s)) });

		let painter = egui_glow::Painter::new(Arc::clone(&glow_context), "", config.shader_version, config.dithering)
	...
	pub fn render(
		...
		let context = window.gl_context().expect("failed to get baseview gl context");
		unsafe {
			context.make_current();
		}

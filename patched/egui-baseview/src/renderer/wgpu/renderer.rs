...
use egui_wgpu::{
	wgpu::{
		Color, CommandEncoderDescriptor, Extent3d, Instance, InstanceDescriptor, RenderPassColorAttachment,
		RenderPassDescriptor, Surface, SurfaceConfiguration, SurfaceTargetUnsafe, TextureDescriptor, TextureDimension,
...
use raw_window_handle_06::{
	AppKitDisplayHandle, AppKitWindowHandle, Win32WindowHandle, WindowsDisplayHandle, XcbDisplayHandle, XcbWindowHandle,
...
impl Renderer {
	pub fn new(window: &Window, config: GraphicsConfig) -> Result<Self, WgpuError> {
		...
		let target = SurfaceTargetUnsafe::RawHandle {
			raw_display_handle: match raw_display_handle {
				...
				raw_window_handle::RawDisplayHandle::Xlib(handle) => raw_window_handle_06::RawDisplayHandle::Xlib(
					XlibDisplayHandle::new(NonNull::new(handle.display), handle.screen),
				),
				raw_window_handle::RawDisplayHandle::Xcb(handle) => raw_window_handle_06::RawDisplayHandle::Xcb(
			...
			raw_window_handle: match raw_window_handle {
				raw_window_handle::RawWindowHandle::AppKit(handle) => {
					raw_window_handle_06::RawWindowHandle::AppKit(AppKitWindowHandle::new(NonNull::new(handle.ns_view).unwrap()))
				}
				raw_window_handle::RawWindowHandle::Xlib(handle) => {
					raw_window_handle_06::RawWindowHandle::Xlib(XlibWindowHandle::new(handle.window))
				}
				raw_window_handle::RawWindowHandle::Xcb(handle) => {
					raw_window_handle_06::RawWindowHandle::Xcb(XcbWindowHandle::new(NonZeroU32::new(handle.window).unwrap()))
				}
	...
	pub fn max_texture_side(&self) -> usize {
		self.render_state.as_ref().device.limits().max_texture_dimension_2d as usize
	}
	...
	fn configure_surface(&self, width: u32, height: u32) {
		...
		if let Some(desired_maximum_frame_latency) = self.config.wgpu_options.desired_maximum_frame_latency {
			surf_config.desired_maximum_frame_latency = desired_maximum_frame_latency;
		}

		self.surface.configure(&self.render_state.device, &surf_config);
	}
	...
	pub fn render(
		...
		let user_cmd_bufs = {
			...
			for (id, image_delta) in &full_output.textures_delta.set {
				renderer.update_texture(&self.render_state.device, &self.render_state.queue, *id, image_delta);
			}
		...
		if self.width != canvas_width || self.height != canvas_height || self.msaa_texture_view.is_none() {
...

		{
			...
			let frame_view = output_frame.texture.create_view(&TextureViewDescriptor::default());
